...
(() => {
  ...
  const state = {
    ...
    mediaTrackers: new Map(),
    mediaSaveTimers: new Map()
  };
      ...
      if (cloudflareVideoId) {
        return `
          <article class="student-content-block">
            ...
            <div class="student-cloudflare-video">
              <div
                class="student-cloudflare-player"
                data-progress-media="cloudflare"
                data-cloudflare-video-id="${esc(cloudflareVideoId)}"
                data-block-id="${esc(block.id || '')}"
                data-video-title="${esc(block.title || 'Course video')}"
              ></div>
              <div class="student-media-progress" data-media-progress-label>
                Video progress: 0%
              </div>
            </div>
          </article>
  ...
  async function getCloudflarePlaybackUrl(videoId) {
    ...
    if (!cleanId) throw new Error('Cloudflare Stream Video ID is missing.');
    ...
    const { data, error } = await client.functions.invoke(
      ...
      { body: { videoId: cleanId } }
    );

    if (error) throw error;
    if (!data?.success || !data?.embedUrl) {
      throw new Error(data?.error || 'Unable to load Cloudflare Stream video.');
    }

    return data.embedUrl;
  }

  function getStoredMediaPosition() {
    const progress = getLessonProgress(state.currentLesson?.id);
    return Math.max(0, Number(progress?.last_position_seconds || 0));
  }

  function updateMediaProgressLabel(element, percent) {
    const label = element.closest('.student-cloudflare-video, .student-content-block')
      ?.querySelector('[data-media-progress-label]');
    if (label) label.textContent = `Video progress: ${percent}%`;
  }

  function getPlayedCoverage(media, duration) {
    const total = Number(duration || 0);
    const played = media?.played;

    if (!total || !played || typeof played.length !== 'number') return 0;

    let covered = 0;
    for (let index = 0; index < played.length; index += 1) {
      try {
        covered += Math.max(0, played.end(index) - played.start(index));
      } catch (_) {}
    }
  ...
  function queueMediaProgressSave(media, currentTime, duration, source) {
    if (!state.currentLesson?.id || !state.enrollment?.id) return;

    const seconds = Math.max(0, Number(currentTime || 0));
    const total = Math.max(0, Number(duration || 0));
    const positionPercent = total > 0
      ? Math.min(100, Math.round((seconds / total) * 10000) / 100)
      : 0;
    const watchedPercent = getPlayedCoverage(media, total);
    const percent = Math.min(99.9, Math.max(positionPercent, watchedPercent));

    updateMediaProgressLabel(media, Math.round(Math.max(watchedPercent, positionPercent)));

    const lessonId = state.currentLesson.id;
    const existing = state.lessonProgress.get(lessonId) || {};
    const existingPosition = Number(existing.last_position_seconds || 0);

    state.lessonProgress.set(lessonId, {
      ...existing,
      progress_percent: Math.max(Number(existing.progress_percent || 0), percent),
      last_position_seconds: Math.max(existingPosition, seconds)
    });

    clearTimeout(state.mediaSaveTimers.get(lessonId));

    const save = async () => {
      const latest = state.lessonProgress.get(lessonId);
      if (!latest) return;

      const payload = {
        enrollment_id: state.enrollment.id,
        lesson_id: lessonId,
        progress_percent: Math.min(99.9, Number(latest.progress_percent || 0)),
        last_position_seconds: Math.max(0, Number(latest.last_position_seconds || 0)),
      ...
      if (error) {
        console.error(`Unable to save ${source || 'media'} progress:`, error);
        return;
      }
    ...
    state.mediaSaveTimers.set(
      lessonId,
      setTimeout(() => save().catch(console.error), 1500)
    );
  }

  async function completeAfterVerifiedPlayback(media, duration, source) {
    const watchedPercent = getPlayedCoverage(media, duration);

    queueMediaProgressSave(media, duration, duration, source);

    if (watchedPercent < 95) {
      toast('Keep watching until at least 95% of the video has been viewed.', 'error');
      return;
    }
  ...
  function attachNativeMediaProgress(media) {
    const restore = () => {
      const position = getStoredMediaPosition();
      if (position > 0 && Number.isFinite(media.duration) && position < media.duration - 1) {
        try { media.currentTime = position; } catch (_) {}
      }
    };
    ...
    media.addEventListener('timeupdate', () => {
      ...
      queueMediaProgressSave(media, media.currentTime, media.duration, 'native video');
    });
    ...
    media.addEventListener('ended', async () => {
      await completeAfterVerifiedPlayback(media, media.duration, 'native video');
    });
  }

  async function attachCloudflareMediaProgress(container) {
    const videoId = container.dataset.cloudflareVideoId;

    try {
      const iframe = document.createElement('iframe');
      iframe.title = container.dataset.videoTitle || 'Course video';
      iframe.allow = 'accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture;';
      iframe.allowFullscreen = true;
      iframe.style.cssText = 'border:0;width:100%;height:100%;';

      const embedUrl = await getCloudflarePlaybackUrl(videoId);
      iframe.src = embedUrl;
      ...
      if (!window.Stream) {
        await new Promise((resolve, reject) => {
          ...
          script.onerror = () => reject(new Error('Unable to load Cloudflare Stream Player SDK.'));
          document.head.appendChild(script);
        });
      }

      const player = window.Stream(iframe);
      const tracker = { player, iframe, lastSavedAt: 0 };
      state.mediaTrackers.set(videoId, tracker);

      const restore = () => {
        const position = getStoredMediaPosition();
        if (position > 0 && Number.isFinite(player.duration) && position < player.duration - 1) {
          try { player.currentTime = position; } catch (_) {}
        }
      };
      ...
      player.addEventListener('timeupdate', () => {
        const now = Date.now();
        if (now - tracker.lastSavedAt < 5000) return;
        tracker.lastSavedAt = now;
        queueMediaProgressSave(iframe, player.currentTime, player.duration, 'Cloudflare Stream');
      });
      ...
      player.addEventListener('ended', async () => {
        await completeAfterVerifiedPlayback(player, player.duration, 'Cloudflare Stream');
      });
    } catch (error) {
      console.error(error);
      container.innerHTML = `<div class="student-media-error">${esc(error.message || 'Unable to load this video.')}</div>`;
    }
  }

  function attachMediaProgressTracking() {
    document.querySelectorAll('video[data-progress-media="native"]').forEach(attachNativeMediaProgress);
    document.querySelectorAll('[data-progress-media="cloudflare"]').forEach(attachCloudflareMediaProgress);
  }
  ...
  function bindEvents() {
    ...
    window.addEventListener('pagehide', () => {
      document.querySelectorAll('video[data-progress-media="native"]').forEach(media => {
        queueMediaProgressSave(media, media.currentTime, media.duration, 'native video');
      });

      state.mediaTrackers.forEach(({ player, iframe }) => {
        queueMediaProgressSave(iframe, player.currentTime, player.duration, 'Cloudflare Stream');
      });
    });
  }
