...
(() => {
  ...
  function blockForm(block) {
    ...
    if (['video','audio','image','pdf','download'].includes(type)) return common + `
      <label>Media URL
  ...
  async function uploadMedia(file, blockType) {
    ...
    /* Private bucket: keep the storage path in the block.
       Student/admin clients should request signed URLs when
       rendering protected media. */
    return media;
  }
